...
const Hero = () => {
  ...
  useEffect(() => {
    ...
    const shapes = Array.from({ length: 15 }, (_, i) => ({
      ...
      color: ['from-blue-400 to-cyan-500', 'from-emerald-400 to-teal-500', 'from-orange-400 to-red-500', 'from-pink-400 to-rose-500', 'from-amber-400 to-yellow-500', 'from-sky-400 to-blue-500'][i % 6]
    }));
  ...
  const orbs = [
    { icon: Users, title: 'Elite Talent', color: 'from-blue-400 to-cyan-500', size: 'w-20 h-20', glow: 'shadow-blue-500/50' },
    { icon: Target, title: 'Strategic Match', color: 'from-emerald-400 to-teal-500', size: 'w-16 h-16', glow: 'shadow-emerald-500/50' },
    { icon: Rocket, title: 'Growth Focus', color: 'from-orange-400 to-red-500', size: 'w-24 h-24', glow: 'shadow-orange-500/50' },
    { icon: Star, title: 'Top Performers', color: 'from-pink-400 to-rose-500', size: 'w-18 h-18', glow: 'shadow-pink-500/50' },
    { icon: TrendingUp, title: 'Innovation', color: 'from-amber-400 to-yellow-500', size: 'w-14 h-14', glow: 'shadow-amber-500/50' },
    { icon: Award, title: 'Excellence', color: 'from-sky-400 to-blue-500', size: 'w-22 h-22', glow: 'shadow-sky-500/50' }
  ];
    ...
    <div ref={heroRef} className="min-h-screen bg-gradient-to-br from-gray-900 via-black to-gray-800 relative overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden">
        ...
        {[Instagram, Twitter, Youtube, Linkedin, Facebook, Hash, AtSign, Send, Bell, Heart, Share2, Eye].map((Icon, index) => (
          ...
          >
            <div className={`w-8 h-8 rounded-xl bg-gradient-to-r ${['from-blue-400/20 to-cyan-500/20', 'from-emerald-400/20 to-teal-500/20', 'from-orange-400/20 to-red-500/20'][index % 3]} backdrop-blur-sm border border-white/10`}>
      ...
      <div className="relative z-10 px-4 sm:px-8 pt-24 pb-16">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="text-center mb-20">
            ...
            <div className="space-y-8 mb-12">
              <div className={`transition-all duration-1500 delay-300 ${isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-20 opacity-0'}`}>
                <h1 className="text-5xl sm:text-6xl lg:text-7xl xl:text-8xl font-black leading-tight tracking-tight px-4">
                  <span className="block text-transparent bg-clip-text bg-gradient-to-r from-blue-400 via-cyan-500 to-blue-600">
                    Where High-Potential Talent
                  </span>
                  <span className="block text-white mt-2">Meets Visionary Brands</span>
                </h1>
              </div>
              ...
              <div className={`transition-all duration-1500 delay-600 ${isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-20 opacity-0'}`}>
                <p className="text-lg sm:text-xl lg:text-2xl text-gray-300 max-w-5xl mx-auto leading-relaxed px-4">
                  We connect high-potential talent with the brands shaping the future. Whether you're a rising star looking to make your mark or a forward-thinking company seeking standout performers, we create meaningful partnerships that drive growth, innovation, and lasting impact.
                </p>
              </div>
            </div>

            {/* CTA Buttons */}
            <div className={`flex flex-col sm:flex-row gap-6 justify-center mb-16 transition-all duration-1500 delay-900 px-4 ${isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-20 opacity-0'}`}>
              <button className="group relative px-8 sm:px-12 py-5 bg-gradient-to-r from-blue-500 to-cyan-600 text-white font-bold text-lg sm:text-xl rounded-2xl hover:from-blue-600 hover:to-cyan-700 transition-all duration-300 shadow-2xl hover:shadow-blue-500/50 hover:scale-105 overflow-hidden">
              ...
              <button className="group px-8 sm:px-12 py-5 bg-gray-800/80 text-white font-bold text-lg sm:text-xl rounded-2xl hover:bg-gray-700/80 transition-all duration-300 border-2 border-gray-600/50 hover:border-blue-400/50 flex items-center justify-center space-x-3 hover:scale-105 backdrop-blur-sm">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-6 sm:gap-8 items-center justify-items-center opacity-60 px-4">
            {['Netflix', 'Spotify', 'Nike', 'Apple', 'Google', 'Meta'].map((brand, index) => (
              <div
                key={brand}
                className="text-gray-400 font-bold text-base sm:text-lg md:text-xl hover:text-gray-200 transition-all duration-300 cursor-pointer hover:scale-110"
